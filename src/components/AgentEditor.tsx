...
const MODEL_CATALOG: Record<Provider, string[]> = {
  'Ollama': ['Gemma3-4B','Qwen3-7B','Qwen3-14B','gpt-oss-20B'],
        ...
        <section className="rounded-2xl border p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              {isCustom ? (
                <input className="mt-1 w-full rounded-xl border p-2" value={config.model} placeholder="例: my-local-gemma3" onChange={(e) => onChange({ ...config, model: e.target.value })} />
              ) : (
                <select className="mt-1 w-full rounded-xl border p-2" value={config.model} onChange={(e) => onChange({ ...config, model: e.target.value })}>
            ...
            {(config.provider === 'LM Studio' || config.provider === 'vLLM' || config.provider === 'OpenAI互換URL') && (
              <>
                <div>
                  ...
                  <input className="mt-1 w-full rounded-xl border p-2" placeholder="http://localhost:1234/v1" value={config.endpoint ?? ''} onChange={(e) => onChange({ ...config, endpoint: e.target.value })} />
                </div>
                <div>
                  <label className="text-xs text-gray-600">API Key（必要時）</label>
                  <input className="mt-1 w-full rounded-xl border p-2" placeholder="sk-..." value={config.apiKey ?? ''} onChange={(e) => onChange({ ...config, apiKey: e.target.value })} />
                </div>
              </>
            )}
            <div>
              <label className="text-xs text-gray-600">温度</label>
              <input type="range" min={0} max={1} step={0.01} value={config.temperature} onChange={(e) => onChange({ ...config, temperature: Number(e.target.value) })} className="w-full" />
              <div className="text-xs text-gray-500">{config.temperature.toFixed(2)}</div>
            </div>
            <div>
              <label className="text-xs text-gray-600">top_p</label>
              <input type="range" min={0} max={1} step={0.01} value={config.top_p} onChange={(e) => onChange({ ...config, top_p: Number(e.target.value) })} className="w-full" />
              <div className="text-xs text-gray-500">{config.top_p.toFixed(2)}</div>
            </div>
            <div>
              <label className="text-xs text-gray-600">max_tokens</label>
              <input type="number" className="mt-1 w-full rounded-xl border p-2" value={config.max_tokens} onChange={(e) => onChange({ ...config, max_tokens: Number(e.target.value) })} />
            </div>
            <div>
              <label className="text-xs text-gray-600">repetition_penalty</label>
              <input type="number" step="0.01" className="mt-1 w-full rounded-xl border p-2" value={config.repetition_penalty} onChange={(e) => onChange({ ...config, repetition_penalty: Number(e.target.value) })} />
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3">
            <div>
              ...
              <select className="mt-1 w-full rounded-xl border p-2" value={config.failover} onChange={(e) => onChange({ ...config, failover: e.target.value as Failover })}>
                {(['OFF','SOFT','HARD'] as Failover[]).map((f) => <option key={f} value={f}>{f}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-gray-600">timeout_s</label>
              <input type="number" className="mt-1 w-full rounded-xl border p-2" value={config.timeout_s} onChange={(e) => onChange({ ...config, timeout_s: Number(e.target.value) })} />
            </div>
            <div>
              <label className="text-xs text-gray-600">min_tps</label>
              <input type="number" className="mt-1 w-full rounded-xl border p-2" value={config.min_tps} onChange={(e) => onChange({ ...config, min_tps: Number(e.target.value) })} />
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              ...
              <textarea className="mt-1 w-full rounded-xl border p-2 h-20" value={config.promptSystem} onChange={(e) => onChange({ ...config, promptSystem: e.target.value })} />
            </div>
            <div className="col-span-2">
              <label className="text-xs text-gray-600">Style</label>
              <textarea className="mt-1 w-full rounded-xl border p-2 h-16" value={config.promptStyle} onChange={(e) => onChange({ ...config, promptStyle: e.target.value })} />
            </div>
          </div>
        </section>

        <section className="rounded-2xl border p-4">
          <h3 className="font-semibold text-sm mb-3">単体テスト</h3>
          <div className="flex gap-2">
            <input className="flex-1 rounded-xl border p-2" placeholder="テスト入力…" value={testInput} onChange={(e)=>setTestInput(e.target.value)} />
            <button
              className="rounded-xl bg-black text-white px-3 py-2 disabled:opacity-50"
              disabled={busy}
              onClick={async ()=>{
                setBusy(true); setTestOutput('');
                try {
      ...
      <div className="col-span-4 space-y-4">
        <section className="rounded-2xl border p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="rounded-lg border px-2 py-1 text-xs" onClick={() => onChange({ ...config, rtt: Math.round(100 + Math.random() * 100), tps: Math.round(6 + Math.random() * 6) })}>
              疎通テスト
            </button>
          </div>
